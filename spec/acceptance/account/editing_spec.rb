...
feature "Account Editing" do
  ...
  scenario "User can edit password and name with correct credentials" do
    ...
    EditAccountPage.open.fill_form(user_name: self.user2.name,
                  password: self.user2.password,
                  password_confirmation: self.user2.password,
                  current_password: self.user1.password).submit_form
    ...
    LoginPage.open.fill_form(email: self.user1.email,
                  password: self.user2.password).submit_form
    expect(HomePage).to be_authenticated
    expect(EditAccountPage.open.form_data).to eq({user_name: self.user2.name,
  ...
  scenario "User can edit email with correct credentials" do
    self.user2 = build(:user)
    log_in_as(self.user1)
    EditAccountPage.open.fill_form(user_name: self.user1.name,
  ...
  scenario "User can not edit account with existing email" do
    self.user2 = build(:user)
    SignUpPage.open.sign_up_as(self.user2.name, self.user2.email, self.user2.password)
    ...
    EditAccountPage.open.fill_form(email: self.user1.email,
  ...
  scenario "User can not edit account with incorrect password" do
    self.user2 = build(:user)
    log_in_as(self.user1)
    EditAccountPage.open.fill_form(password: self.user2.password,
                  password_confirmation: self.user2.password,
