...
feature "Article Editing" do
  ...
  scenario "User can not edit article with blank title", :p1 => true do
    ...
    EditArticlePage.on do
      ...
      submit_form
      expect(errors_section.error_message).to eql("2 errors prohibited this article from being saved: Title can't be blank Title is too short (minimum is 5 characters)")
    end
  end
  ...
  scenario "User can not edit article with title is too short", :p1 => true do
    ...
    EditArticlePage.on do
      ...
      submit_form
      expect(errors_section.error_message).to eql("1 error prohibited this article from being saved: Title is too short (minimum is 5 characters)")
    end
  end
end
